...
export default async function PMCalibrationPage() {
  ...
  const { data: pmSchedules } = await supabase
    .from("pm_schedules")
    .select("id, task_name, next_due, equipment_id, equipment_description, location, equipment(tag_number, name)")
    .order("next_due");
            ...
            <tbody>
              {pmSchedules.map((pm: any) => {
                ...
                const equipmentLabel = pm.equipment?.tag_number
                  ...
                  : [pm.equipment_description, pm.location].filter(Boolean).join(" · ") || "—";
                return (
